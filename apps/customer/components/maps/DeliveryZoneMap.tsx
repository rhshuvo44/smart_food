...
// Color palette for zone fills (semi-transparent)
const ZONE_COLORS = [
  'rgba(255, 107, 53, 0.15)', // primary
  'rgba(0, 78, 137, 0.15)', // secondary
  'rgba(40, 167, 69, 0.15)', // success
  'rgba(255, 193, 7, 0.15)', // warning
...
export default function DeliveryZoneMap({
    ...
    return (
      <View style={[styles.emptyContainer, style]}>
        ...
        <Text style={styles.emptySubtext}>
          Delivery zones will appear here once added by an admin.
        </Text>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, style]}>
      <MapView initialRegion={defaultRegion}>
        {zones.map((zone, index) => {
          const colorIndex = index % ZONE_COLORS.length;
          // Extract the outer ring from the GeoJSON Polygon
          ...
          {zones.map((zone, index) => {
            ...
            return (
              <View key={zone.id} style={styles.legendItem}>
                <View
                  style={[styles.legendColor, { backgroundColor: ZONE_STROKE_COLORS[colorIndex] }]}
                />
